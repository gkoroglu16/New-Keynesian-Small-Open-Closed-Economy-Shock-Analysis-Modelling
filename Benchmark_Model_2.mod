% Benchmark_Model_2.mod
%
% DESCRIPTION: dynare.mod file to solve question 2
%
% Group 30
%
...
var c_hat i_hat pi_hat y_hat s_hat q_hat pi_h_hat pi_f_hat mc_hat a_hat
    ...
    epsilon_z_hat epsilon_s_hat deltay_hat ca_hat e_hat epsilon_ystar
    epsilon_pistar epsilon_istar ;

varexo epsilon_m_hat eta_phi eta_z eta_s eta_h eta_ystar
       eta_pistar eta_istar;

parameters sig alp eta bet bet_star theta_h chi phi h gamma delta_h rho_i
...
model (linear);
    ...
    %Specificiation 2 (16B - 18B)
    //16B
    y_star_hat = y_star_hat(+1)-(i_star_hat-pi_star_hat(+1))+epsilon_ystar;

    //17B
    pi_star_hat = bet_star*pi_star_hat(+1)+kappa_star*y_star_hat+epsilon_pistar;

    //18B
    i_star_hat = rho_istar*i_star_hat(-1)+(1-rho_istar)*(psi_pi_star*pi_star_hat
                 +psi_y_star*y_star_hat)+epsilon_istar;

    //19
    epsilon_ystar = rho_ystar*epsilon_ystar(-1)+eta_ystar;

    //20
    epsilon_pistar = rho_pistar*epsilon_pistar(-1)+eta_pistar;

    //21
    epsilon_istar = eta_istar;

    %End of Specification 2
...
stoch_simul(irf=12, order=1, irf_shocks=(eta_istar), nograph) y_hat c_hat i_hat pi_hat ec_hat i_star_hat e_hat ca_hat;
